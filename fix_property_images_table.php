<?php
/**
 * Fix Property Images Table ID Column
 * This script fixes the property_images table to ensure the ID column is AUTO_INCREMENT PRIMARY KEY
 */
...
echo "=== Fixing Property Images Table ===\n\n";

// First, check if the property_images table exists
$check_table = "SHOW TABLES LIKE 'property_images'";
$result = mysqli_query($conn, $check_table);

if (mysqli_num_rows($result) == 0) {
    echo "ERROR: Property_images table does not exist!\n";
    exit(1);
}

echo "✓ Property_images table exists\n";

// Check current structure of the ID column
$check_column = "SHOW COLUMNS FROM property_images LIKE 'id'";
...
if (mysqli_num_rows($result) == 0) {
    ...
    $alter_query = "ALTER TABLE property_images ADD COLUMN id INT AUTO_INCREMENT PRIMARY KEY FIRST";
    ...
    if ($column_info['Key'] == 'PRI' && strpos($column_info['Extra'], 'auto_increment') !== false) {
        ...
        $check_zero_ids = "SELECT COUNT(*) as count FROM property_images WHERE id = 0";
        ...
        if ($row['count'] > 0) {
            ...
            // Get the maximum ID
            $max_id_query = "SELECT COALESCE(MAX(id), 0) as max_id FROM property_images WHERE id > 0";
            $max_result = mysqli_query($conn, $max_id_query);
            $max_row = mysqli_fetch_assoc($max_result);
            $next_id = $max_row['max_id'] + 1;
            
            // Update all records with ID = 0
            $update_query = "UPDATE property_images SET id = ? WHERE id = 0 LIMIT 1";
        ...
        $count_query = "SELECT COUNT(*) as count FROM property_images";
        ...
        if ($record_count > 0) {
            ...
            $create_temp = "CREATE TABLE property_images_temp LIKE property_images";
            ...
            $alter_temp = "ALTER TABLE property_images_temp MODIFY COLUMN id INT AUTO_INCREMENT PRIMARY KEY FIRST";
            if (!mysqli_query($conn, $alter_temp)) {
                echo "ERROR: Failed to alter temp table: " . mysqli_error($conn) . "\n";
                mysqli_query($conn, "DROP TABLE property_images_temp");
                exit(1);
            }
            echo "✓ Temporary table created\n";
            
            // Step 2: Get all columns except ID
            $columns_query = "SHOW COLUMNS FROM property_images";
            ...
            $copy_data = "INSERT INTO property_images_temp ($columns_str) SELECT $columns_str FROM property_images";
            ...
            mysqli_query($conn, "DROP TABLE property_images");
            mysqli_query($conn, "RENAME TABLE property_images_temp TO property_images");
            echo "✓ Table replaced successfully\n";
            
            echo "\n✓ Property_images table fixed successfully!\n";
            ...
            if ($column_info['Key'] == 'PRI') {
                ...
                $drop_pk = "ALTER TABLE property_images DROP PRIMARY KEY";
                mysqli_query($conn, $drop_pk);
            }
            
            // Modify ID column to be AUTO_INCREMENT PRIMARY KEY
            $alter_query = "ALTER TABLE property_images MODIFY COLUMN id INT AUTO_INCREMENT PRIMARY KEY FIRST";
...
// Verify the fix
echo "\n=== Verification ===\n";
$verify_query = "SHOW COLUMNS FROM property_images WHERE Field = 'id'";
...
if ($verify_row['Key'] == 'PRI' && strpos($verify_row['Extra'], 'auto_increment') !== false) {
    ...
    // Show sample of IDs
    $sample_query = "SELECT id, property_id, image_url FROM property_images ORDER BY id LIMIT 5";
    $sample_result = mysqli_query($conn, $sample_query);
    
    if (mysqli_num_rows($sample_result) > 0) {
        echo "\nSample property image IDs:\n";
        while ($sample = mysqli_fetch_assoc($sample_result)) {
            echo "  - Image ID: " . $sample['id'] . " (Property ID: " . $sample['property_id'] . ", Image: " . substr($sample['image_url'], 0, 30) . "...)\n";
        }
    }
} else {
